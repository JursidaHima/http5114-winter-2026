# Course Content

---

## Module 1 – Course Introduction

### Topics
- Course introduction
- Midterm and Final grading (SAT and UNSAT)
- Introduction to BrickMMO
- Basic BrickMMO demo
- Register for Discord, GitHub, Stack Overflow, and Flow
- Complete Discord, GitHub, Stack Overflow, and Flow profiles


---

## Module 2 – GitHub

### Topics
- Creating repositories
- Clone, add, commit, push, pull
- Installing GitHub CLI
- Installing GitHub Desktop



---

## Module 3 – Markdown

### Topics
- Introduction to Markdown
- Headings, links, images, lists, code blocks, warnings
- _readme project
- GitHub Profile README.md
- Editing GitHub profile directly on GitHub (no CLI)
- Using _readme tool


---

## Module 4 – GitHub in Groups

### Topics
- Forking
- Pull requests
- Branching
- Merging
- Adding name, links, and profile to contributions repository



---

## Module 5 – Coding from Scratch: Accordion

### Topics
- Build a JavaScript Accordion from scratch

---

## Module 6 – Deployment

### Topics
- GitHub Pages
- Deployment to shared hosting
- Demo deployment to GCP and AWS


---

## Module 7 – LEGO Challenge

### Topics
- Build an input/output device using LEGO Spike

---

## Module 8 – Stop, Start, Continue

### Topics
- Stop, Start, Continue survey
- Monty Hall problem

---

## Module 9 – Coding from Scratch: Slideshow

### Topics
- Build a JavaScript Slideshow from scratch

---

## Module 10 – Monty Hall

### Topics
- Coding logic exercise

---

## Module 11 – HTML Portfolio

### Topics
- Work on final project assignment
- Use Stack Overflow for research and solutions



---

## Module 12 – Coding from Scratch: Contact Form

### Topics
- Work on final project assignment
- Use Stack Overflow for research and solutions



---

## Module 13 – APIs and Open Weather Map

### Topics
- Working with APIs
- Open Weather Map integration
- Stop, Start, Continue survey
- Course wrap-up


---

## Module 14 – Review

### Topics
- Answer questions
- Help with assignments
- Course content review

### Assignment
- Stack Overflow
